...
public class PDFBarcodeController extends BaseController {
  ...
  public PDFBarcodeController(FileService fs) {
    this.fs = fs;
  }


  /**
   * Endpoint for EAN8 Barcode generation on each page
   *
   * @param pdfFile
   * @return
   * @throws Exception
   */
  @PostMapping(value = "/barcode/page",
      consumes = MediaType.MULTIPART_FORM_DATA_VALUE,
      produces = MediaType.APPLICATION_PDF_VALUE)
  public ResponseEntity<byte[]> barcodeOnEachPage(@RequestPart("file") MultipartFile pdfFile) throws Exception {
    ...
    return getPDFDocResponseEntity(dest, "barcode-page.pdf");
  }


  /**
   * Generate a PDF containing all barcodes
   *
   * @return
   * @throws Exception
   */
  @GetMapping(value = "barcode/all", produces = MediaType.APPLICATION_PDF_VALUE)
  public ResponseEntity<byte[]> generateBarcode() throws Exception {
    File tempDir = fs.getTempDir();
    File dest = new File(tempDir, getRandomPdfName());

    PdfDocument pdfDoc = new PdfDocument(new PdfWriter(dest));
    try (Document doc = new Document(pdfDoc, new PageSize(340, 842))) {
    ...
    return getPDFDocResponseEntity(dest, "barcode-types.pdf");
  }
